$(document).ready(function () {
    ...
    $('body').on('click', '.quick-add-product-to-cart', function (e) {
        e.preventDefault();
        var el = $(this);
		// var attribute=$(this).parents('.product-card').find('input.product-attribute-value:checked');
		var attribute=$(this).parents('.product-card').find('input:checked');
		console.log($(attribute).val());
        var data = {'Product[id]': el.data('product-id'),'Product[attribute_id]':$(attribute).val()};
    ...
    function changePositionQuantity(productId, quantity) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result) {
                    updateCartWidget();
					$.pjax.reload({container : '#cart-items-wrapper', "timeout" : 100000,'push':true});
                }
            }
        });
    }
    ...
    $(document).on('click','.cart-delete-product',function (e) {
        e.preventDefault();
        var el = $(this);
    ...
    $(document).on('change','.position-count',function () {
